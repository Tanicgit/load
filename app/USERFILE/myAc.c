...
{
	uint8_t ucCRCHi = crc_in>>8;
	uint8_t ucCRCLo = crc_in;
	int iIndex;
	while( usDataLen-- )
	{
			iIndex = ucCRCLo ^ *( puchMsg++ );
...
_myAc Bin2HexStr(uint32_t bin,uint8_t bits,char* ret)
{
	...
	for(i=0;i<bits;i++)
	{
		if((ret[i] = BinTo_09_AF(bin>>((bits-i+1)*4))&0xf)==0)return AC_ERR;
	}
	return AC_OK;
}
...
_myAc HexStr2Bins(char *HexStr,uint16_t StrLen,uint8_t *re)
{
	...
	{
		a = _09_af_AF_ToBin(HexStr[2*i]);
		b = _09_af_AF_ToBin(HexStr[2*i+1]);
		if(a==0xff||b==0xff)return AC_ERR;
		re[i] = a<<4 | b;
	}
	return AC_OK;
}
