...
QByteArray app::process(const QByteArray &data)
{
...
QByteArray app::ErrAck(int e, uint8_t *data)
{
    _MSG7f_t msg7f_t;
...
int app::MsgPro(uint8_t *data,uint16_t len,QByteArray *re)
{
...
QByteArray app::MsgPacked(uint8_t *a,uint16_t len)
{
    QByteArray re;
    uint8_t sum=0;
    re.append('@');
...
bool app::updataBinFile(QByteArray bindata, uint16_t ver, uint32_t size, uint16_t pSize)
{
...
char app::BinTo_09_AF(uint8_t a)
{
...
uchar app::_09_af_AF_ToBin(char a,bool *ok)
{
    *ok=true;
    if(a>=0x30 && a<=0x39)
    {
...
bool app::HexStr2Bins(uchar *HexStr,uint16_t StrLen,uint8_t *re)
{
    bool ok;
...
uint8_t app::U8checkSum(uint8_t *a,uint8_t len)
{
    uint8_t i=0,sum=0;
    for(i=0;i<len;i++)
    {
...
uint16_t app::Crc16_Modbus(uint8_t *puchMsg,uint16_t usDataLen)
{
    uint16_t i=0;
    uint16_t j=0;
